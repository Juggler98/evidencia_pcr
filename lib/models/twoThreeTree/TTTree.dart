import 'TTTreeNode.dart';
import 'TTTreeSonType.dart';
...
class TTTree<K extends Comparable<K>, T extends Comparable<T>> {
  ...
  */
  ...
  bool tryToAdd(T newData) {
    if (root == null) {
      root = TTTreeNode<K, T>(newData);
      ++height;
      return true;
    }

    var leaf = this.findLeaf(newData);
...
/*
  ...
  getInOrderDataInterval(TTTreeNode<K, T> node, T start, T end, bool left) {
    List<T> data = [];
    if (node == null) {
      return data;
    }
...
/*
  ...
  */
   ...
   */
  T removeData(T data) {
    var node = searchNodeWithData(data);
    if (node == null) {
      //print("Mazanie, prvok neexistuje: " + data.getKey());
      return null;
    }
    T deletedData;
    bool left;
  ...
  bool tryToRemove(TTTreeNode<K, T> node, bool leftToDelete) {
    if (node.isLeaf()) {
      if (!node.isThreeNode()) {
        if (node == this.root) {
          this.root = null;
    ...
    while (true) {
      ...
      var inOrderLeafSonType = getSonType(inOrderLeaf);
  ...
  getSonType(TTTreeNode<K, T> node) {
    if (node != null) {
